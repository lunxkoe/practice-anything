package authsystem.security.session.registry;

import authsystem.security.core.session.dto.UserSession;
import authsystem.security.core.session.exception.business.RefreshTokenReusedException;
...
public class UserSessionRedisRegistry implements UserSessionRegistry {
    ...
    redisTemplate.execute(new SessionCallback<Object>() {
      ...
      public Object execute(RedisOperations operations) {
        ...
        operations.opsForZSet()
            .add(indexKey, session.sessionId().toString(), expiresAt.toEpochMilli());
  ...
  private void refreshIndexExpiry(String indexKey, Instant candidateExpiresAt) {
    ...
    if (highest != null && !highest.isEmpty()) {
      ...
      if (maxScore != null) {
        maxExpiresAt = Instant.ofEpochMilli(
            Math.max(candidateExpiresAt.toEpochMilli(), maxScore.longValue()));
      }
    }
